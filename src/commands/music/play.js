...
module.exports = class PlayCommand extends BaseCommand{
    ...
    async run (para){
        ...
        const searchResults = await bot.music
            .search(query, msg.author)
            .catch((err) => {console.log(err)});
        
        // checks if there any search result(s), if not return a message to inform the author
        if(!searchResults) return msg.channel.send(`**${author}**-sama, Aqukin can't find any tracks with the given keywords, please also note that Aqukin will not queue Youtube Audio Mix or Stream`, para.ridingAqua);;
