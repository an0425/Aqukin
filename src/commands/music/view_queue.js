...
module.exports = class ViewQueueCommand extends BaseCommand {
  ...
  async run (para) {
    ...
    if (player.songs.length === 0 ) { return message.channel.send(`**${message.author.username}**-sama, Aqukin the queue is currently empty~`, para.ridingAqua); }
    ...
    const embeds = generateQueueEmbed(player.songs, thumbnails);
