...
module.exports = class HelpCommand extends BaseCommand{
    constructor() {super("help", [], "Provide info about commands", "SEND_MESSAGES", "utility", false, false, "[command]")}
    ...
    async run(para) {
        ...
        if(para.args.length){ // if the author has asked for help with a specific command
            ...
            */
            ...
            const musicCmdNames = musicCmd.map(cmd => cmd.name.toString()).join("\n");
            const econCmdNames = econCmd.map(cmd => cmd.name.toString()).join("\n");
            const utilsCmdNames = utilsCmd.map(cmd => cmd.name.toString()).join("\n");
