...
module.exports = class MusicStuckEvent extends BaseEvent {
    ...
    async run (bot, player, track, payload) {
        ...
        await await bot.channels.cache.get(player.textChannel).send(`**${track.requester.username}**-sama, \`${payload}\` has occured when ${bot.user.username} was trying to play track \`${track.title}\` 。 ゜ ゜ (´Ｏ\`) ゜ ゜。`);
    } // end of run
