...
module.exports = class ViewQueueCommand extends BaseCommand {
    ...
    async run (para) {
        ...
        const embeds = await generateQueueEmbed(player.queue, thumbnails, gifs, para.bot.music.lavalink);
...
async function generateQueueEmbed(queue, thumbnails, gifs, lavalink) {
    const embeds = [];
